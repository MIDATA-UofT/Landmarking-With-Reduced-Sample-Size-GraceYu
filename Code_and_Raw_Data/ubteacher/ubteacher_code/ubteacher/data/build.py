...
def divide_label_unlabel(
    ...
    labeled_size = 208
    # this is the number of labelled dataset we have, and also the maximum of valid index for labelled dataset

    # read from pre-generated data seed
    # TODO: I have changed the below 3 lines.
    ...
    num_label = int(SupPercent / 100. * num_all)
    # Here I use num_label instead to make sure index within the range of labelled data is generated
    # labeled_idx = np.random.choice(range(num_all), size=num_label, replace=False)
    #TODO: I added the above.
    labeled_idx = np.random.choice(range(labeled_size), size=num_label, replace=False)
